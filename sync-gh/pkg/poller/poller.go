...
func NewPoller() *Poller {
	...
	if token != "" {
		...
		log.Printf("Using unauthenticated GitHub API (60 req/hour). Set GITHUB_TOKEN for higher limits.")
	}
...
func (p *Poller) checkRepo(repo string, config RepoConfig) error {
	...
	if err != nil {
		log.Printf("Could not read current version for %s: %v", config.Subsystem, err)
		return nil
	}
